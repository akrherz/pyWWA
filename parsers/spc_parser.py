...
def real_parser(txn, buf):
        ...
        for z in wfos[cat]:
            for wfo in z:
                msgs[wfo] = ("The Storm Prediction Center issues Day 1 %s "
                             "risk for portions of %s %s"
                             ) % (cat, wfo, url)
                htmlmsgs[wfo] = ("<p>The Storm Prediction Center issues "
    ...
    for wfo in msgs.keys():
        xtra['channels'] = wfo
        jabber.sendMessage(msgs[wfo], htmlmsgs[wfo], xtra.copy())
